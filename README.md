...
PyEnv Doctor 是一个智能的 Python 环境诊断与自动修复工具。它通过构建依赖图谱、在沙箱中预演修复方案，确保修复建议的安全性。v1.1 版本新增**快照管理**、**自动修复**和**回滚引擎**，实现完整的环境治理闭环。
...
- 快照管理：一键创建环境快照，记录所有包的版本信息
- 自动修复：`diagnose --fix` 自动执行修复，支持三种策略
- 回滚引擎：快速恢复到任意快照状态，支持预演和验证
- 临时快照：自动修复时自动创建临时快照，修复成功后可转为永久

#### 原有功能

- 智能诊断：自动扫描环境，检测依赖冲突
- 沙箱预演：在隔离环境中验证修复方案，确保安全
- 多格式支持：支持诊断当前环境或检查 requirements.txt/pyproject.toml 文件
- 报告导出：支持 JSON 和 Markdown 格式报告
- 并行优化：多线程并行预演，提升诊断速度
...
当前最新版本 `1.1.3` 已发布到 TestPyPI，包含完整的快照管理、自动修复和回滚功能。
